...
int main(int argc, char *argv[]){
  ...
  if (pass != NUM_THREADS*2) {
    ...
    perror("*** test-27-8t-fine-mixed failed: file Overflow FOUND when it should NOT be! ***");
    return 1;
  }
  ...
  for (int i = 1; i <= NUM_THREADS; ++i) {
    ...
    if (system(cmd)) {
      fprintf(stderr, "*** test-27-8t-fine-mixed failed: file Id-%d not found ***\n", i);
      return 1;
    }
  }
  ...
  for (int i = NUM_THREADS+1; i <= 9; ++i) {
    ...
    if (!system(cmd)) { // These should fail!
      fprintf(stderr, "*** test-27-8t-fine-mixed failed: file Id-%d FOUND when it should NOT be! ***\n", i);
      return 1;
    }
  }

  printf("*** test-27-8t-fine-mixed PASSED ***\n");
  return 0;
}
