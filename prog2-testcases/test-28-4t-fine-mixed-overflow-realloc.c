...
int main(int argc, char *argv[]){
  ...
  if (pass != NUM_THREADS*2) {
    printf("*** test-28-4t-fine-mixed failed: (only %d threads passed)\n", pass);
    return 1;
  }

    if (system("ls Overflow")) {
    perror("*** test-28-4t-fine-mixed failed: file Overflow not found ***");
    return 1;
  }
  ...
  for (int i = 1; i <= NUM_THREADS; ++i) {
    ...
    if (system(cmd)) {
      fprintf(stderr, "*** test-28-4t-fine-mixed failed: file Id-%d not found ***\n", i);
      return 1;
    }
  }
...
base = "ls Id-%d >/dev/null 2>&1";
  for (int i = NUM_THREADS+1; i <= 9; ++i) {
    ...
    if (!system(cmd)) { // These should fail!
      fprintf(stderr, "*** test-28-4t-fine-mixed failed: file Id-%d FOUND when it should NOT be! ***\n", i);
      return 1;
    }
  }

  printf("*** test-28-4t-fine-mixed-overflow PASSED ***\n");
  return 0;
}
