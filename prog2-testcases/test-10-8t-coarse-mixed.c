...
/**
 * 8 threads that, together, (almost) exhaustively allocate all small & large
 * chunks.
*/
void * test(void * args) {
  long fail = 0;
  ...
  void ** small_chunks = malloc(NUMS_64 / NUM_THREADS * sizeof(void *));
  void ** large_chunks = malloc(NUMS_1024 / NUM_THREADS * sizeof(void *));
  ...
  for (int i = 0; i < NUMS_64 / NUM_THREADS; ++i) {
  ...
  for (int i = 0; i < NUMS_1024 / NUM_THREADS; ++i) {
...
int main(int argc, char *argv[]) {
  ...
  printf("*** test-10-8t-coarse-mixed PASSED ***\n");
  return 0;
}
