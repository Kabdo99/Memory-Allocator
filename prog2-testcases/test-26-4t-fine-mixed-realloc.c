...
#define SMALL_COUNT 1572
#define LARGE_COUNT 132
...
/**
 * Using 4 threads, allocate all local small and large blocks, then release,
...
int main(int argc, char *argv[]){
  ...
  if (pass != NUM_THREADS*2) {
    printf("test failed (%d threads passed)\n", pass);
    return 1;
  }

  if (!system("ls Overflow >/dev/null 2>&1")) { // This should fail!
    perror("*** test-26-4t-fine-mixed failed: file Overflow FOUND when it should NOT be! ***");
    return 1;
  }
  ...
  for (int i = 1; i <= NUM_THREADS; ++i) {
    ...
    if (system(cmd)) {
      fprintf(stderr, "*** test-26-4t-fine-mixed failed: file Id-%d not found ***\n", i);
      return 1;
    }
  }
...
base = "ls Id-%d >/dev/null 2>&1";
  for (int i = NUM_THREADS+1; i <= 9; ++i) {
    ...
    if (!system(cmd)) { // These should fail!
      fprintf(stderr, "*** test-26-4t-fine-mixed failed: file Id-%d FOUND when it should NOT be! ***\n", i);
      return 1;
    }
  }

  printf("*** test-26-4t-fine-mixed PASSED ***\n");
  return 0;
}
