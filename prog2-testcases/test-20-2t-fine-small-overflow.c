...
/**
 * Forked from testcase 14; threads will need to reach the global overflow list
*/
void * test(void * args) {
  char buf[64];
...
int main(int argc, char *argv[]) {
  ...
  if (verdict) {
    perror("*** test-20-2t-fine-small-overflow failed: thread 0 exited with a non-zero status ***\n");
    return 1;
  }
  pthread_join(t1, (void *) &verdict);
  if (verdict) {
    perror("*** test-20-2t-fine-small-overflow failed: thread 1 exited with a non-zero status ***\n");
    return 1;
  }

  if (system("ls Overflow")) {
    perror("*** test-20-2t-fine-small-overflow failed: file Overflow not found ***");
    return 1;
  }
  ...
  for (int i = 1; i <= NUM_THREADS; ++i) {
    ...
    if (system(cmd)) {
      fprintf(stderr, "*** test-20-2t-fine-small-overflow failed: file Id-%d not found ***\n", i);
      return 1;
    }
  }
...
base = "ls Id-%d >/dev/null 2>&1";
  for (int i = NUM_THREADS+1; i <= 9; ++i) {
    ...
    if (!system(cmd)) { // These should fail!
      fprintf(stderr, "*** test-20-2t-fine-small-overflow failed: file Id-%d FOUND when it should NOT be! ***\n", i);
      return 1;
    }
  }

  puts("*** test-20-2t-fine-small-overflow PASSED ***");
  return 0;
}
